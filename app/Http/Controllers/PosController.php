            ...
            foreach ($request->payments as $index => $payment) {
                if (isset($payment['amount']) && $payment['amount'] > 0) {
                    ...
                    if ($payment['payment_method_id'] == 1 || $payment['payment_method_id'] == '1') {
                        ...
                        if (!$existing) {
                            // No prior Stripe customer: must use a per-line token to create the customer
                            ...
                            // Prefer per-line selected card or default fallback
                            $selectedCardId = $payment['selected_card_id'] ?? ($selectedCardsByLine[$index] ?? $request->card_id);
                            $tokenLine = $cardTokensByLine[$index] ?? null;

                            if ($tokenLine) {
                                // New card for this line
                                $card = \Stripe\Customer::createSource($customerStripeId, ['source' => $tokenLine]);
                                $charge = \Stripe\Charge::create([
                                    'amount' => $paymentAmount * 100,
                                    'currency' => 'usd',
